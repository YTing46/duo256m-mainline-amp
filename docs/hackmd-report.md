...
kernel patch 系列（v7.0-rc6 起 24 個 patch，可完整重現）與韌體修改（5 個 patch）
...
改 hard-IRQ 直送後 **RTT ~127-131µs（boot-to-boot 變異）、p99 ~180µs——
反超手工 glue 逾 20µs**。直送需配套整條 rx 路徑的 irq-safety 改造
（mutex→spinlock、GFP_ATOMIC、NS 建立延遲到 work）。p99 尾巴經機率驗證
= kernel timer tick（HZ=250、131µs 視窗命中率 3.3% ↔ p97 起飛），非 bug。
